...
def init_llm_trace_run(doc):
    ...
    base_dir = run_root / f"{run_title}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    run_dir = base_dir
    suffix = 1
    while run_dir.exists():
        run_dir = run_root / f"{base_dir.name}_{suffix:03d}"
        suffix += 1
    run_dir.mkdir(parents=True, exist_ok=False)

    _LLM_TRACE_LOCAL.state = {
        "run_dir": str(run_dir),
        "counter": 0,
        "lock": threading.Lock(),
    }
...
def clear_llm_trace_run():
    if hasattr(_LLM_TRACE_LOCAL, "state"):
        delattr(_LLM_TRACE_LOCAL, "state")


def _get_llm_trace_state():
    return getattr(_LLM_TRACE_LOCAL, "state", None)
...
def _get_caller_stack_trace(max_frames=8):
    stack = traceback.extract_stack()
    filtered = []
    for frame in reversed(stack):
        # Keep frames from project files and skip logger internals.
        if "pageindex" not in frame.filename and not frame.filename.endswith(
            "run_pageindex.py"
        ):
            continue
        if frame.filename.endswith("utils.py") and frame.name in {
            "llm_completion",
            "llm_acompletion",
    ...
    if not filtered:
        return "No project caller stack available"
...
def _write_llm_trace_file(messages, response, model, is_async, retries=0, error=None):
    ...
    content = (
        f"#### LLM CALL {call_id:04d} ####\n"
        f"Timestamp: {datetime.now().isoformat()}\n"
        f"Model: {model}\n"
        f"Async: {'yes' if is_async else 'no'}\n"
        f"Retries: {retries}\n"
...
def ensure_parent_ranges_cover_children(nodes):
    """
    Ensure each parent node's [start_index, end_index] covers all descendants.

    This is a conservative normalization pass: it only widens parent ranges when
    children fall outside the parent bounds. It never shrinks ranges.
    """
    ...
    def _normalize(node):
        ...
        if not children:
            return node.get("start_index"), node.get("end_index")

        child_starts = []
        child_ends = []
        for child in children:
            c_start, c_end = _normalize(child)
        ...
        if child_starts:
            ...
            if own_start is None or min_child_start < own_start:
                node["start_index"] = min_child_start

        if child_ends:
            max_child_end = max(child_ends)
            if own_end is None or max_child_end > own_end:
